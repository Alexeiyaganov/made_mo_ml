...
class LineSearchTool(object):
    ...
    def line_search(self, oracle, x_k, d_k, previous_alpha=None):
        ...
        alpha : float or None if failure
            Chosen step size
        """
        def armijo(c1, alpha_0=1):
            alpha = previous_alpha or alpha_0
            phi0 = oracle.func(x_k)
            phi_0 = oracle.grad(x_k).T @ d_k
            while oracle.func(x_k + alpha * d_k) > phi0 + c1 * alpha * phi_0 and alpha > 1e-20:
                alpha /= 2.
            return alpha
        if self._method == 'Constant':
            return self.c
        elif self._method == 'Armijo':
            return armijo(self.c1, self.alpha_0)
        elif self._method == 'Wolfe':
            return scipy.optimize.linesearch.scalar_search_wolfe2(lambda x: oracle.func(x_k + x * d_k),
                                                                  lambda x: oracle.grad(x_k + x * d_k).T @ d_k,
                                                                  phi0=oracle.func(x_k), c1=self.c1, c2=self.c2)[0] or\
                   armijo(self.c1)
