...
def gd(filename, x_init, A, y, gamma, 
    ...
    for it in range(S):
        ...
        x = x - gamma * logreg_grad_plus_lasso(x, [A, y, l2, sparse, l1]) 
...
def FISTA(filename, x_init, A, y, L, 
    ...
    for it in range(S):
        ...
        #ваш код здесь (возможно, ещё где-то придётся вставить код)
        grad_w = logreg_grad(yk, [A, y, mu, sparse])
        xk1 = prox_R(yk - grad_w / L, l1 / L)
        yk = xk1 + (np.sqrt(k) - 1) * (xk1 - xk) / (np.sqrt(k) + 1)
...
def prox_gd(filename, x_init, A, y, gamma, 
    ...
    for it in range(S):
        #ваш код здесь
        grad_w = logreg_grad(w, [A, y, l2, sparse])
...
def svrg(filename, x_init, A, y, gamma, 
    ...
    for s in range(S):
        ...
        grad_w = logreg_grad(w, [A, y, l2, sparse_full, l1])
        for it in range(M): 
            ...
            batch_ind = indices[indices_counter:(indices_counter+batch_size)]
            indices_counter += batch_size
            g_k = logreg_grad(x, [A_for_batch[batch_ind], y[batch_ind], l2, sparse_stoch]) - logreg_grad(w, [A_for_batch[batch_ind], y[batch_ind], l2, sparse_stoch]) + grad_w
            x = prox_R(x - gamma * g_k, gamma * l1)
            #ваш код здесь
            num_of_data_passes += 2.0*batch_size/m
